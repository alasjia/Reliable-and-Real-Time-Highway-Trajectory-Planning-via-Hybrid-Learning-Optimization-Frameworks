'''
in this version, maintain the old name "traj_pred_mlp" (instead of "vel_pred_mlp").
this is for using the old version of .pth of velocity prediction model, "VN_parameters_4_display.pth".
because i don't know somehow the new trained model output very bad velocity profiles of EV.
'''
...
class VectorNet(nn.Module):
    ...
    def forward(self, data):
        ...
        v_norm = self.traj_pred_mlp(global_feat) # [batch_size, 160]       #应该是 [128 9, 160]
...
if __name__ == "__main__":
    ...
    torch.save(
        model.state_dict(),
        os.path.join(save_dir_train, f"vel_pred_VN_{epochs}ep.pth")
    )

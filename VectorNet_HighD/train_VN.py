'''
2025.12.29: velocity prediction is changed to trajectory prediction

next task is to operate this extensively on "chwei"
'''
...
# from HighD_datapre_ import MyOwnDataset
...
def train_epoch(model, data_loader, device , criterion, optimizer, pred_len, scheduler):
    ...
    for batch in data_loader:
        ...
        output = model(batch)    
        y_hat = output['pred']    #(128, 30, 2)   
        B = batch.num_graphs  # 128
        y_gt = batch.y.view(B, pred_len, 2).to(torch.float32)   # [B,T,2]
        #均采用mse loss  
...
def eval_epoch(model, data_loader, device, criterion, pred_len):
    ...
    with torch.no_grad():
        for batch in data_loader:
            ...
            err = torch.norm(y_hat - y_gt, dim=-1)  # [B,T]   每个时间步的瞬时位置误差
    ...
    rmse = np.sqrt(mean_mse)  #这是坐标值的均方根误差，而不是位移的均方根误差。
    ade = total_ade / batch_num
    fde = total_fde / batch_num
    return rmse, ade, fde
...
def loss_plot_broken_yaxis(train_loss_ls, dev_loss_ls, test_loss, train_loss, dev_loss, epochs,
    ...
    if not use_broken_axis:
        ...
        ax.set_ylabel('RMSE (m)', labelpad=10)
        ...
        ax_top.set_ylabel('RMSE (m)', labelpad=10)
...
if __name__ == "__main__":
    ...
    torch.save(
        model.state_dict(),
        os.path.join(save_dir_train, f"traj_pred_VN_{epochs}ep_lrdf09.pth")
    )
...
<2026.2.14>
epoch 95: train_rmse 0.8962 | dev_rmse 0.9090 m | ADE 0.7612 m | FDE 1.7735 m
epoch 96: train_rmse 0.8962 | dev_rmse 0.9057 m | ADE 0.7610 m | FDE 1.7732 m
epoch 97: train_rmse 0.8962 | dev_rmse 0.9052 m | ADE 0.7610 m | FDE 1.7731 m
epoch 98: train_rmse 0.8962 | dev_rmse 0.9054 m | ADE 0.7610 m | FDE 1.7732 m
epoch 99: train_rmse 0.8962 | dev_rmse 0.9043 m | ADE 0.7608 m | FDE 1.7729 m
训练运行时间: 21777.881921 秒
test loss: 0.9026 m, last train loss: 0.8962, last dev loss: 0.9043 m 


[03/10/2026]
epoch 95: train_rmse 0.8842 | dev_rmse 0.9066 m | ADE 0.7627 m | FDE 1.7696 m
epoch 96: train_rmse 0.8841 | dev_rmse 0.8986 m | ADE 0.7501 m | FDE 1.7515 m
epoch 97: train_rmse 0.8837 | dev_rmse 0.9029 m | ADE 0.7549 m | FDE 1.7559 m
epoch 98: train_rmse 0.8839 | dev_rmse 0.8938 m | ADE 0.7476 m | FDE 1.7478 m
epoch 99: train_rmse 0.8841 | dev_rmse 0.9002 m | ADE 0.7510 m | FDE 1.7516 m
训练运行时间: 39967.750499 秒
test loss: 0.8924 m, last train loss: 0.8841, last dev loss: 0.9002 m 
'''

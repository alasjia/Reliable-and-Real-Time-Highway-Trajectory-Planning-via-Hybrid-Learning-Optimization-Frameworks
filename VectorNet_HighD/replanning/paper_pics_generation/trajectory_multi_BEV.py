...
def draw_trajectory_multi_BEV_e2e(
    ...
    color_ev = "#ED746A"
    # Hybrid v2 uses 8 fixed colors for SV1..SV8 (no modulo!)
    colors_sv = ['#638DEE', '#66C999', '#F49568', '#82C61E',
                '#E0A4DD', '#AA66EB', '#77DCDD', '#DBAA77']
        ...
        for k in range(1, bevs_per_sub + 1):
            ...
            if 0 <= idx_sv < sv_gt.shape[0]:
                for sv_id in range(MAX_SV):
                    ...
                    vx_c = sv_gt[idx_sv, 6 * sv_id + 2]
                    vy_c = sv_gt[idx_sv, 6 * sv_id + 3]

                    if np.isnan(x_c) or np.isnan(y_c):
                        continue

                    # --- SV heading (match Hybrid v2 spirit) ---
                    # Hybrid v2: np.degrees(np.arctan(vy/vx)) :contentReference[oaicite:5]{index=5}
                    # Use arctan2 to avoid division-by-zero and sign issues:
                    sv_angle = float(np.degrees(np.arctan2(vy_c, vx_c))) if (not np.isnan(vx_c) and not np.isnan(vy_c)) else 0.0
                    ...
                    if k == 1:
                        ...
                        face_col = grads_sv[sv_id][k - 2]   # 注意：这里不再取模
